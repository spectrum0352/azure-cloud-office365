...
- Rotate access keys (App Registrations, Service Principals) every 90
  days
...
- Enable diagnostic logs on all critical services (Key Vault, SQL,
  Storage, etc.)
...
# Azure Security Checks for IAM Hardening 
...
**☁️ Access Control to Azure Resources**
...
- Test if sensitive operations (e.g., role assignments, key access) are
  logged.

- Try to identify gaps in log coverage (e.g., no logs for VMs or SQL).

#  Azure Logging & Auditing Security Checklist
...
Would you like this exported to a JSON or Markdown file for automation
or reporting use?

# Networking Security

These checks are inspired by AWS best practices but rewritten in the
context of Azure environments. They help identify overly permissive
networking configurations and logging gaps commonly targeted in lateral
movement or external exposure.
...
Would you like this checklist combined with the previous logging and IAM
checklists into a single Azure audit playbook?

# Monitoring & Alerting Security Checklist
...
Would you like this merged into a consolidated **Azure security audit
playbook**?
